...
The RPM spec file provided here is for Ellocash-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://ellocash.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Ellocash git repository](https://github.com/ellocash/ellocash) but are not part of
...
Currently only `Source10` is used, for the example `ellocash.conf` file.
...
in the ellocash git repository. At present only an SVG version of the Ellocash
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the ellocash
...
needed when 0.12.1 is released, a proper fix is already in the Ellocash git
master branch.
...
idea to build Ellocash Core against a static version of that library compiled
...
The `ellocash.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `ellocash-server` package. At this point, most RPM based
...
The `ellocash-server` package should have SELinux support. How to properly do
...
Ellocash-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/ellocash/). He has been
packaging Ellocash for Fedora at least since 2012.
...
4. I renamed the `ellocash` package that contains the Qt GUI to `ellocash-core` as
that appears to be how the general population refers to it, in contrast to
`ellocash-xt` or `ellocash-classic`. I wanted to make sure the general population
