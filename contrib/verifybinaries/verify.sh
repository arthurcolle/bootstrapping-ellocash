...
###   ellocashcore.org and ellocash.org and compares them.
...
WORKINGDIR="/tmp/ellocash_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://ellocashcore.org"
HOST2="https://ellocash.org"
BASEDIR="/bin/"
VERSIONPREFIX="ellocash-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'ellocash-',
...
if [ $? -ne 0 ]; then
   echo "ellocash.org failed to provide signature file, but ellocashcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "ellocash.org and ellocashcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Ellocash Core binary release signing key installed?"
   fi
