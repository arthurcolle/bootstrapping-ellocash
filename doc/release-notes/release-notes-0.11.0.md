Ellocash Core version 0.11.0 is now available from:

  <https://ellocash.org/bin/ellocash-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer LOC/kB fee to be
...
For example, add the following to `ellocash.conf`:
...
related to the blockchain in the ellocash system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Ellocash Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Ellocash Core will 
...
For an example script see [ellocash-submittx](https://github.com/laanwj/ellocash-submittx).
...
- #5334 `cf87536` libellocashconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new ellocash-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only EllocashAmountField
- #5753 `0f44672` Add ellocash logo to about screen
...
- #5809 `46bfbe7` Add ellocash-cli man page
...
- #5849 `41259ca` contrib/init/ellocashd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Ellocash-Qt.app to Ellocash-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for ellocash-tx)
