 ...
 * between ellocashd, and ellocash-qt and non-server tools.
*/
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), ELLOCASH_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/ellocash/ellocash>";
    const std::string URL_WEBSITE = "<https://ellocashcore.org>";
...
/** Sanity checks
 *  Ensure that Ellocash is running in a usable environment with all
...
bool AppInitParameterInteraction()
{
    ...
    if (gArgs.GetBoolArg("-disablefork", false)) {
        printf("NOTE: disabling fork; this node will run on the ellocash network\n");
        FORK_ALLOWED = false;
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Ellocash process is using the data directory.
...
// ********************************************************* Step 4a: application initialization
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", ELLOCASH_CONF_FILENAME)).string());
