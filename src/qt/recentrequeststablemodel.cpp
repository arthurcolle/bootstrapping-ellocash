    ...
    if (role == Qt::DisplayRole || role == Qt::EditRole) {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return EllocashUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, EllocashUnits::separatorNever);
            else
                return EllocashUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
...
{
    return (this->walletModel->getOptionsModel() != nullptr) ? tr("Requested") + " (" + EllocashUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
